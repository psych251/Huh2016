...
#### Importing the Data
```{r}
setwd("~/Desktop/PSYCH 251 - Stats/Pilot")
...
d_tidy_sim <- d %>%
        gather(sim, sim_rating, starts_with("sim")) %>%
        group_by(sim) %>%
        summarise(mean_sim = mean(sim_rating, na.rm = TRUE))
...
d_tidy_pref <- d %>%
        ...
        separate(pref, c("set_pref", "item"), "_") %>%
...
d_tidy_pref$set_pref <- gsub("pref", "", d_tidy_pref$set_pref)
...
# cleaning data. 0 = cross category, 1 = within category
d_tidy_choice <- d %>%
...
d_tidy <- d %>%
        ...
        gather(set, choice, starts_with("sub")) %>%
        gather(sim_set, sim_rating, starts_with("sim")) %>%
        gather(pref, pref_rating, starts_with("pref")) %>%
